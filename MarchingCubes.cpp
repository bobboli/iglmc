...
void MarchingCubes::polygonise(int i, int j, int k){
	...
	//if( vertexCount+3 < maxVertexCount ){
		...
		if (edgeTableOurs[cubeindex] == 0)		return;
...
{
	for (i = 0; triTableOurs[cubeindex][i] != -1; i += 3)
	{
		//if(bSmoothed){
		//	//smoothed normals
		...
		vertices.push_back(vertList[triTableOurs[cubeindex][i]]);
		vertices.push_back(vertList[triTableOurs[cubeindex][i + 1]]);
		vertices.push_back(vertList[triTableOurs[cubeindex][i + 2]]);
...
{
	if (edgeTableOurs[cubeindex] & 1)		vertexInterp(threshold, i, j, k, i1, j, k, vertList[0], normList[0]);
	if (edgeTableOurs[cubeindex] & 2)		vertexInterp(threshold, i1, j, k, i1, j1, k, vertList[1], normList[1]);
	if (edgeTableOurs[cubeindex] & 4)		vertexInterp(threshold, i1, j1, k, i, j1, k, vertList[2], normList[2]);
	if (edgeTableOurs[cubeindex] & 8)		vertexInterp(threshold, i, j1, k, i, j, k, vertList[3], normList[3]);
	if (edgeTableOurs[cubeindex] & 16)		vertexInterp(threshold, i, j, k1, i1, j, k1, vertList[4], normList[4]);
	if (edgeTableOurs[cubeindex] & 32)		vertexInterp(threshold, i1, j, k1, i1, j1, k1, vertList[5], normList[5]);
	if (edgeTableOurs[cubeindex] & 64)		vertexInterp(threshold, i1, j1, k1, i, j1, k1, vertList[6], normList[6]);
	if (edgeTableOurs[cubeindex] & 128)		vertexInterp(threshold, i, j1, k1, i, j, k1, vertList[7], normList[7]);
	if (edgeTableOurs[cubeindex] & 256)		vertexInterp(threshold, i, j, k, i, j, k1, vertList[8], normList[8]);
	if (edgeTableOurs[cubeindex] & 512)		vertexInterp(threshold, i1, j, k, i1, j, k1, vertList[9], normList[9]);
	if (edgeTableOurs[cubeindex] & 1024)	vertexInterp(threshold, i1, j1, k, i1, j1, k1, vertList[10], normList[10]);
	if (edgeTableOurs[cubeindex] & 2048)	vertexInterp(threshold, i, j1, k, i, j1, k1, vertList[11], normList[11]);
}
...
void MarchingCubes::polygonise_level_stage3(int level)
{
	...
	for (int y = 0; y < sy; ++y)
	{
		for (int z = 0; z < sz; ++z)
		{
			...
			int* triTableEntry = triTableOurs[cubeIndexLevel[iCube]];
			for (int ti = 0; triTableEntry[ti] != -1; ti += 3)
			{
				for (int tj = 0; tj < 3; tj++)
				{
					int val = triTableOurs[cubeIndexLevel[iCube]][ti + tj];
...
void MarchingCubes::polygonise_level_stage2(int level, int* vertIndexZOld, int* vertIndexYOld, int x, int* vertIndexZNew, int* vertIndexYNew)
{
	...
	if (level == 0)
	{
		for (int z = 0; z < sz; ++z)
		{
			if (edgeTableOurs[cubeIndexLevel[z]] & 256)
			{
		...
		for (int y = 0; y < sy; ++y)
		{
			if (edgeTableOurs[cubeIndexLevel[y * sz]] & 8)
			{
		...
		for (int y = 0; y < sy; ++y)
		{
			for (int z = 0; z < sz; ++z)
			{
				int edgeIndex = edgeTableOurs[cubeIndexLevel[y * sz + z]];
				if (edgeIndex & 128)
				{
	...
	{
		{
			if (edgeTableOurs[cubeIndexLevel[0]] & 1)
			{
		...
		for (int y = 0; y < sy; ++y)
		{
			for (int z = 0; z < sz; ++z)
			{
				...
				int edgeIndex = edgeTableOurs[cubeIndex];
				if (edgeIndex & 64)
				{
	...
	#define VERTEX_INTERP_SIMD 0
		...
		for (int y = 0; y < sy; ++y)
		{
...

			{
				...
				for (int z = 0; z < sz; ++z)
				{
					edgeIndexArray[z] = edgeTableOurs[cubeIndexLevel[iCube++]];
				}
			}
			...
			*/
			...
			for (int z = 0; z < sz; ++z)
			{
				...
				// (Though a little bit strange to put it here).
				{
					...
					int base = iCube + y;
					int cubeIndex = cubeIndexLevel[iCube];
					int* triTableEntry = triTableOurs[cubeIndex];
					for (int i = 0; triTableEntry[i] != -1; ++i)
					{
						int val = triTableEntry[i];
						int idx = base + offsetLookUp[val + (level & 1) * 12];
...
void MarchingCubes::polygonise_count_ops(int i, int j, int k, operation_counts& counts)
{
	...
	if (edgeTableOurs[cubeindex] == 0)
	{
		counts.empty_cells += 1;
		return;
	}

	if (edgeTableOurs[cubeindex] & 1)		vertexInterp_count_ops(threshold, i, j, k, i1, j, k, counts);
	if (edgeTableOurs[cubeindex] & 2)		vertexInterp_count_ops(threshold, i1, j, k, i1, j1, k, counts);
	if (edgeTableOurs[cubeindex] & 4)		vertexInterp_count_ops(threshold, i1, j1, k, i, j1, k, counts);
	if (edgeTableOurs[cubeindex] & 8)		vertexInterp_count_ops(threshold, i, j1, k, i, j, k, counts);
	if (edgeTableOurs[cubeindex] & 16)		vertexInterp_count_ops(threshold, i, j, k1, i1, j, k1, counts);
	if (edgeTableOurs[cubeindex] & 32)		vertexInterp_count_ops(threshold, i1, j, k1, i1, j1, k1, counts);
	if (edgeTableOurs[cubeindex] & 64)		vertexInterp_count_ops(threshold, i1, j1, k1, i, j1, k1, counts);
	if (edgeTableOurs[cubeindex] & 128)		vertexInterp_count_ops(threshold, i, j1, k1, i, j, k1, counts);
	if (edgeTableOurs[cubeindex] & 256)		vertexInterp_count_ops(threshold, i, j, k, i, j, k1, counts);
	if (edgeTableOurs[cubeindex] & 512)		vertexInterp_count_ops(threshold, i1, j, k, i1, j, k1, counts);
	if (edgeTableOurs[cubeindex] & 1024)	vertexInterp_count_ops(threshold, i1, j1, k, i1, j1, k1, counts);
	if (edgeTableOurs[cubeindex] & 2048)	vertexInterp_count_ops(threshold, i, j1, k, i, j1, k1, counts);
